...
func TestWeatherAPI_LoadValidate(t *testing.T) {
	...
	if w.BaseURL != "https://api.weatherapi.com/v1" {
		t.Errorf("Load did not set BaseURL correctly: got %q", w.BaseURL)
	}
	if err := w.Validate(); err != nil {
		t.Errorf("unexpected Validate error: %v", err)
	}
...
func TestSetupConfig_Success(t *testing.T) {
	...
	if cfg.WeatherAPI.BaseURL != vars["WEATHER_API_BASE_URL"] {
		// nolint: lll
		t.Errorf("WeatherAPI.BaseURL = %q; want %q", cfg.WeatherAPI.BaseURL, vars["WEATHER_API_BASE_URL"])
	}
}
